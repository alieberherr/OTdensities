...
function expcost_multiply(result,l,r,u,eps,tol=1e-10)
    Threads.@threads for i in 1:size(result)[1]
    	Threads.@threads for j in 1:size(u)[1]
            tmp=0.
    		Threads.@threads for k in 1:size(r)[2]
    			tmp += (l[i, k] - r[j, k])^2
            end
...
function eval_cost(x,y,ucurrent,vcurrent,eps)
    ...
    Threads.@threads for i in 1:size(MKdotv)[1]
...
function sinkhorn(x,y,a,b,eps,numItermax=1e8,threshold=1e-08,evalStep=1000)
    n = size(x)[1]
    m = size(y)[1]
    ...
    while (nit <= numItermax)
        ...
        if (any(isnan,ucurrent) || any(isnan,vcurrent))
            ...
            cost = eval_cost(cost,x,y,uold,vold,eps)
        end
...
eps=10
# cost, residual = sinkhorn(x,x,a,b,eps)
@time sinkhorn(x,x,a,b,eps)
