...
def overlap(phia, phib):
	...
	return np.sqrt(utils.integrate3D(grid, np.abs(vals1)**2*np.abs(vals2)**2))
...
if __name__ == '__main__':
	...
	with open(resultsfile, 'w') as wfile:
		writer = csv.DictWriter(wfile, fieldnames=["Molecule", "Excitation", "Functional", "Lambda1 (squared)", "Lambda2 (squared)"])
		writer.writeheader()
		with open(args.csvfile,'r') as file:
			...
			for line in reader:
				...
				if not failed:
					for i in range(versions):
						writer.writerow({"Molecule": molecule, "Excitation": excitation, "Functional": functional, f"Lambda%i (squared)"%(i+1): Lambda[i]})
